...
MovingAverageFilter movingAverageFilter2(100);
...
void loop(){
  if (roll > setAngle){
    setReverseMotor(voltage);
  } else if (roll < setAngle){
    setForwardMotor(voltage);
  }
  getAngles();
  float omega = get_AngVel();
  float cps = get_AngVelMotor();
  omega = movingAverageFilter2.process(omega);
  omega = movingAverageFilter2.process(omega);
  cps = movingAverageFilter.process(cps);
  cps = movingAverageFilter.process(cps);
...
void setForwardMotor(double val){
  pwm = val*(255/19);
  if (pwm >= 255){ pwm = 255; }
  else if (pwm <= 0) { pwm = 0; }
...
void setReverseMotor(double val){
  long int pwm = val*(255/19);
  if (pwm >= 255){ pwm = 255; }
  else if (pwm <= 0) { pwm = 0; }
